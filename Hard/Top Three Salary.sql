...
Create Table Employeeee (id int, Name varchar(20), Salart int, DepartmentID int)
...
AS
(
Select *, DENSE_RANK() OVER(Partition by DepartmentID order by Salart desc) RNO from Employeeee
)
SELECT b.Name, A.Name, A.Salart from CTE A JOIN Department B ON A.DepartmentID = b.ID
Where RNO <= 3